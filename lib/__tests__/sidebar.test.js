...
test('write sidebar', () => {
  ...
  expect(lines[0]).toBe('目次');
  expect(lines[1]).toBe('');
  expect(lines[2]).toBe('* [TOP](./TOP)  ');
  expect(lines[3]).toBe('* chapter1  ');
  expect(lines[4]).toBe('    - [仕様1](chapter1/仕様1)  ');
  expect(lines[5]).toBe('* chapter2  ');
  expect(lines[6]).toBe('    - [仕様2](chapter2/仕様2)  ');
  expect(lines[7]).toBe('    - 章2.1  ');
  expect(lines[8]).toBe('        - [あいうえお](chapter2/章2.1/あいうえお)  ');
});
...
test('write sidebar wiki', () => {
  ...
  const mdtext = fs.readFileSync(mdfiles[0], 'utf8');
  const lines = mdtext.split(/[\n]/);
  expect(lines[2]).toBe('* [TOP](./TOP.md)  ');
});
