  ...
  const relationshipRows = Object.values(entities)
    ...
    .reduce((rows, entity, index, array) => {
      Object.values(entity.table.foreignKeys).forEach((fk) => {
        const relationship = tableManager.getReferences(entity.table, fk.id);
...
function createColumnRow(erd, table, column) {
  ...
  // FKの編集
  // 編集例） FK(アカウント, ID)
  const foreignKey = tableManager.getForeignKeysByColumnId(table, column.id);
  if (foreignKey != null) {
    const relationship = tableManager.getReferences(table, foreignKey.id);
    if (relationship !== null) {
      attr.push(
        'FK(' 
        + getEntityName(erd, relationship.refTable)
        + ', '
        + relationship.refColumns
            .map((column) => {
