...
function getColumnName(erd, field) {
  if (a5conf.erdLabelType.LOGICAL === erd.labelType) {
    return field.name;
  } else if (a5conf.erdLabelType.PHYSICAL === erd.labelType) {
    return field.id;
...
function getPropertiesByColumnType(columnType, erd, table) {
  ...
  Object.values(table.fields).forEach((field) => {
    if (!tableManager.isPkField(table, field.id)) {
      return;
    }
    properties.push(createColumnRow(erd, table, field));
  });
  ...
  Object.values(table.fields).forEach((field) => {
    ...
    if (columnType === a5conf.erdColumnType.PK_UK &&
      !tableManager.isUkField(table, field.id)) {
      return;
    }
    properties.push(createColumnRow(erd, table, field));
  });
...
function createColumnRow(erd, table, field) {
  // カラム名の編集
  let columnName = getColumnName(erd, field);
  let attr = [];
  // PKの編集
  if (tableManager.isPkField(table, field.id)) {
  ...
  // UKの編集
  if (tableManager.isUkField(table, field.id)) {
    columnName = '* ' + columnName;
  }
  // FKの編集
  // 編集例） FK(アカウント, ID)
  const foreignKeys = tableManager.getForeignKeysByColumnId(table, field.id);
    ...
    if (relationship !== null) {
      attr.push(
        ...
        + relationship.refFields
            .map((field) => {
              return getColumnName(erd, field);
            })
            .join(', ')
        + ')'
      );
